...
def zabbix_add(auth_info, **kwargs):
    ...
    try:
        ...
        util.write_log('api').debug(username, "create zabbix host %s scucess" % result[0]['hostids'])
    ...
    try:
        ...
        for i in ret:
            if int(i['maintenanceid']) == where['id']:
                for host in i['hosts']:
                    result.append(app.config['zabbix'].host_status(hostid=host['hostid'], status='0'))          # zabbix 触发报警
...
def zabbix_main_add(auth_info, **kwargs):
    ...
    if '1' not in auth_info['r_id']:
        return json.dumps({'code': 1, 'errmsg': 'you not admin,no cmdb'})
    util.write_log('api').info( 'zabbix_maintenance.create …… …… ……')   # test add by zhouzx
    try:
        data = request.get_json()['params']
        ...
        for i in hostids:
            ...
            # update table server -- host_status=0 .add by zhouzx
            fields = ['id', 'cmdb_hostid', 'hostid', 'host', 'ip']
            zabbix_hosts = app.config['cursor'].get_results('zbhost', fields)
            hostid = [str(zb["hostid"]) for zb in zabbix_hosts]
            cmdb_hostid = [str(zb["cmdb_hostid"]) for zb in zabbix_hosts]
            for (x, y) in zip(hostid, cmdb_hostid):         # zip()函数的作用就是在多个等长的序列中，每次循环都从每个序列分别取出一个元素，之后进行元素的聚合
                if i == x:
                    app.config['cursor'].execute_update_sql('server', {'host_status': '0'},{'id': y})  # status=0 更新到server表，0.gif 是维护状态。
            ret.append(app.config['zabbix'].host_status(hostid=i, status='1'))      # sstatus=1 更新到zabbix平台，是维护状态。 by zhouzx
        util.write_log('api').info(username, 'select zabbix template list success')
        return json.dumps({'code': 0, 'result': ret})
    except:
...
def idc_get(auth_info, **kwargs):
    ...
    username = auth_info['username']
    try:
        output = ['id', 'name', 'idc_name', 'address', 'phone', 'email', 'user_interface', 'user_phone',
                  'rel_cabinet_num', 'pact_cabinet_num']
        data = request.get_json()['params']
...
def graph_get(auth_info, **kwargs):
    ...
    try:
        ...
        result = app.config['cursor'].get_one_result('zbhost', fields, data_where)  # SELECT id,hostid FROM zbhost WHERE cmdb_hostid=33
        ...
        for i in grapsh_id:
            if i['name'] in monitor_name:
                ...
                graph = ZabbixGraph(app.config['zabbix_login_url'], app.config['zabbix_user'],
